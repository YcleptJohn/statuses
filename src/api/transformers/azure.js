...
azure._isRecent = (incident) => moment().subtract(2, 'days').isBefore(moment(incident.isoDate)) && azure._isResolved(incident)
...
azure._extractUpdates = (incident) => {
  ...
  const objecUpdates = updates.map(update => {
    ...
    return { time: moment(time, 'M/D/YYYY h:mm:ss A'), text }
  })
  return objecUpdates
}
